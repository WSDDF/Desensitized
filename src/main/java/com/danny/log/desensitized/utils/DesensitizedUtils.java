...
public class DesensitizedUtils {
    public static String getJson(Object javaBean) {
        ...
        if (null != javaBean) {
            try {
                ...
                Object clone = ObjectUtils.deepClone(javaBean, count);
                ...
                log.error("desensitized failed", e);
            }
        }
        return json;
    }
        ...
        if (desensitized != null) {
            ...
            if (isNotEmpty(isEffictiveMethod)) {
                try {
                    ...
                    log.error("NoSuchMethodException脱敏失败",e);
                } catch (IllegalAccessException e) {
                    log.error("IllegalAccessException脱敏失败",e);
                } catch (InvocationTargetException e) {
                    log.error("InvocationTargetException脱敏失败",e);
                }
            }
        }
    ...
    /**
     * 【身份证号】显示最后四位，其他隐藏。共计18位或者15位，比如：1234*********1234
     *
     * @param idCard
     * @return
     */
    public static String idCardNum(String idCard) {
        ...
        if(length == 18){//18位身份证号
            idCard = sb.replace(10,16, "******").replace(17, 18, "*").toString();
        } else if(length == 15){//15位身份证号
            idCard = sb.replace(8,14, "******").toString();
        }
