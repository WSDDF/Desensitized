...
public class ObjectUtils {
    ...
    public static <T> T deepCloneObject(T obj) {
        ...
        try {
            ...
            log.error("IOException failed", e);
        } catch (ClassNotFoundException e) {
            log.error("ClassNotFoundException failed", e);
        }
        return t;
    }
     ...
     */
    public static Object deepClone(Object objSource, int count) throws InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        if (null == objSource || count == 0) {return null;}
        //是否jdk类型、基础类型、枚举类型
            ...
            try {
                ...
                if (type.isArray()) {//对数组类型的字段进行递归过滤
                    ...
                    for (int i = 0; i < len; i++) {
                        ...
                        Array.set(newArray, i, deepClone(arrayObject, count));
                    }
                    ...
                    while (it.hasNext()) {
                        ...
                        newCollection.add(deepClone(collectionObj, count));
                    }
                    ...
                    for (Object o : set) {
                        ...
                        newMap.put(entry.getKey(), deepClone(mapVal, count));
                    }
                ...
                if (isUserDefinedType(value.getClass())) {
                    field.set(objDes, deepClone(value, count));
                    continue;
                }

            } catch (Exception e) {
                log.error("deepClone failed",e);
                return null;
            }
        }
        return objDes;
    ...
    private static boolean isUserDefinedType(Class<?> clazz) {
        return
                clazz.getPackage() != null
                        ...
                        && !StringUtils.startsWith(clazz.getName(), "javax.")
                        && !StringUtils.startsWith(clazz.getName(), "java.")
                        && !StringUtils.startsWith(clazz.getName(), "sun.");
    }
